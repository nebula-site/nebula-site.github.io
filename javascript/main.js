...
function onMouseLeave() {
 ...
 // Create loading container
        ...
        let angle = 0;
        let radius = 35; // Radius of the orbit

        function animateStars() {
            for (let i = 0; i < stars.length; i++) {
                const star = stars[i];
                const x = radius * Math.cos(angle + (i * Math.PI / 2)); // Offset each star's path
                const y = radius * Math.sin(angle + (i * Math.PI / 2));

                star.style.transform = `translate(${50 + x}px, ${50 + y}px)`; // Translate each star
            }

            angle += Math.PI / 60; // Speed of the animation (controls how fast the stars move)
